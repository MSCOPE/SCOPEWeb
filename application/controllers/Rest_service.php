...
class Rest_service extends CI_Controller {
    ...
    private function checkDate($request_date) {
        if (!preg_match("/GMT/i", $request_date) && !preg_match("/UTC/i", $request_date) && !preg_match("/Z/i", $request_date)) {
            $request_date .= " UTC";
        }
        ...
        if (($ts_req > $ts_server - $valid_date_range) && ($ts_req < $ts_server + $valid_date_range)) {
            return true;
        }
        return false;
    }
